function dx = fnDynamics14DOFVehicleOL(t,x,u_c,p,Vehicle)
...
bsrr=   7000;  
hrcf=   0.25;   %front roll center distance below sprung mass c.g. (m)
...
% xsrf=+z1 - (heave + a*sin(theta) - cf/2*sin(phi) );
% xslf=+z2 - (heave + a*sin(theta) + cf/2*sin(phi) );
...
downforce       = 0*0.5*airDens*liftCoeff*frontalArea*u.^2;
...
w_dot=wy.*u-wx.*v+(1/m)*(Fzslf+Fzsrf+Fzslr+Fzsrr+Fdzlf+Fdzrf+Fdzlr+Fdzrr-downforce)-g*cos(phi).*cos(theta);
