function [problem] = fnInitTrackv2(problem, bPlotTrackDef, generateTrack)
...
%     td.yCarRight = td.yCar + TrackWidth/2*gradient(td.xCar)./(gradient(td.xCar).^2 + gradient(td.yCar).^2).^0.5;
    ...
    Track.sLap = linspace(Track.sLap(1)*1,Track.sLap(end),nPoints);
    ...
    e_yaw = abs(Track.aYaw(end))-2*pi;

    delta_yaw = e_yaw./Track.sLap;

    Track.aYaw  	= cumsum((Track.curv-delta_yaw).*Track.d_sLap);

    % calculate delta y and delta x
    deltaX = (td.xCar(1)*0 - td.xCar(end))./Track.sLap;
    deltaY = (td.yCar(1)*0 - td.yCar(end))./Track.sLap;
    % put these delta x and y into the "new" x and y position
%     td.xCar = td.xCar - deltaX;
%     td.yCar = td.yCar - deltaY;
    td.xCar 	= cumsum(Track.d_sLap.*(cos(Track.aYaw)-deltaX));
    td.yCar 	= cumsum(Track.d_sLap.*(sin(Track.aYaw)+deltaY));

    % calculates the delta heading angle
%     newaYaw = atan(deltaY/deltaX)/nPoints;

    % updates it in the "old" heading angle
    Track.Angle     = Track.aYaw ;
    % then we have the final x and y coordinate
    Track.XCoord 	= td.xCar;
    Track.YCoord 	= td.yCar;
