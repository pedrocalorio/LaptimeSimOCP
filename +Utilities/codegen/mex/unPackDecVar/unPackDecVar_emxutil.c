...
{
  emxFree_real_T(sp, &pStruct->xIdx);
  emxFree_real_T(sp, &pStruct->uIdx);
}
...
{
  emxInit_real_T(sp, &pStruct->xIdx, 2, srcLocation);
  emxInit_real_T(sp, &pStruct->uIdx, 2, srcLocation);
}
...
/* End of code generation (unPackDecVar_emxutil.c) */
